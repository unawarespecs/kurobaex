...
class FoolFuukaCommentParser(
  ...
  companion object {
    ...
    private val B4K_DEFAULT_QUOTE_PATTERN = Pattern.compile("(?:https:\\/\\/)?arch.b4k\\.dev\\/(\\w+)\\/(?:post|thread)\\/(\\d+)\\/?(?:#)?(\\d+)?\\/?")
    ...
    private val ROZEN_ARCANA_QUOTE_PATTERN = Pattern.compile("(?:https:\\/\\/)?archive.palanq\\.win\\/(.*?)\\/(?:thread|post)\\/(\\d+)\\/?(?:#)?(\\d+)?\\/?")
